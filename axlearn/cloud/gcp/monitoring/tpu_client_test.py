...
class TestMetrics(parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        [
            (
                ...
                device.TpuChip.V4,
                {
                    tpu_client.MetricName.TOTAL_MEMORY: [34088157184] * 4,
                    tpu_client.MetricName.MEMORY_USAGE: [8522039296 * i for i in range(4)],
                    tpu_client.MetricName.DUTY_CYCLE_PCT: [0.0, 25.0, 50.0, 100.0],
                },
            ),
            (
                "v5litepod_4",
                device.TpuChip.V5E,
                {
                    tpu_client.MetricName.TOTAL_MEMORY: [17044078592] * 4,
                    tpu_client.MetricName.MEMORY_USAGE: [2130509824 * i for i in range(4)],
                    tpu_client.MetricName.DUTY_CYCLE_PCT: [100.0 / (i + 1) for i in range(4)],
                },
            ),
            (
                "v5p_8",
                device.TpuChip.V5P,
                {
                    tpu_client.MetricName.TOTAL_MEMORY: [102803439616] * 4,
                    tpu_client.MetricName.MEMORY_USAGE: [17044078592 * i for i in range(4)],
    ...
    def test_metrics(self, chip_type: device.TpuChip, responses):
        ...
        with DummyTpuMetricServer.fake_tpu_metrics(responses, V5P_METRICS) as fake_libtpu_addr:
            expected_usage = [
                tpu_client.Usage(i, m, t, d)
                for i, (m, t, d) in enumerate(
                    zip(
                        responses[tpu_client.MetricName.MEMORY_USAGE],
                        responses[tpu_client.MetricName.TOTAL_MEMORY],
                        responses[tpu_client.MetricName.DUTY_CYCLE_PCT],
                    )
                )
            ]
