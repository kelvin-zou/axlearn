...
class Usage:
    ...
    device_id: int
    mem_used_in_bytes: int = 0
    mem_total_in_bytes: int = 0
...
def validate_available_metrics(metric_list: List[MetricName], addr: str = "localhost:8431") -> bool:
    """Validate the available metrics against the supported metrics.

    Args:
        metric_list (List[MetricName]): the metrics to be fetched from the libtpu.
        addr (str): grpc client from libtpu. Defaults to "localhost:8431".
...
def get_chip_metrics(
    chip_type: device.TpuChip, metric_list: List[MetricName], addr: str = "localhost:8431"
) -> List[Usage]:
...
def validate_available_metrics_v2(
    metric_list: List[MetricName], addr: str = "localhost:2112"
) -> bool:
    """Validate the available metrics against the supported metrics."""
...
def get_chip_metrics_v2(
    chip_type: device.TpuChip, metric_list: List[MetricV2Name], addr: str = "localhost:2112"
) -> List[Usage]:
    ...
    Args:
        ...
        metrics_addr: Address of tpu-device-plugin metrics server.
